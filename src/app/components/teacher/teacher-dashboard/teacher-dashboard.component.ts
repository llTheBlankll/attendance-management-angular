...
export class TeacherDashboardComponent implements OnInit {
  ...
  initializeDashboardStatistics() {
    ...
    this.attendanceService.countAttendanceInSection(this.section, new DateRange(currentDate, currentDate), Status.ON_TIME).subscribe((response: HttpResponse<number>) => {
      this.onTimeAttendance = response.body as number;
    });

    this.attendanceService.countAttendanceInSection(this.section, new DateRange(currentDate, currentDate), Status.LATE).subscribe((response: HttpResponse<number>) => {
      this.lateAttendance = response.body as number;
    });

    this.attendanceService.countAttendanceInSection(this.section, new DateRange(currentDate, currentDate), Status.ABSENT).subscribe((response: HttpResponse<number>) => {
